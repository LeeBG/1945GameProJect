...
public class PlayerPlane extends JLabel {
	...
	public void playerDraw(Graphics g) {
		for (int i = 0; i < playerAttackList.size(); i++) {
			...
			if (select == 1) {
				g.drawImage(playerAttack.playerBulletImg1, (int) playerAttack.getX(), (int) playerAttack.getY(),
						null);
			}
			if (select == 2) {
				g.drawImage(playerAttack.playerBulletImg2, (int) playerAttack.getX(), (int) playerAttack.getY(), 20,
						20, null);
			}
			if (select == 3) {
				g.drawImage(playerAttack.playerBulletImg3, (int) playerAttack.getX(), (int) playerAttack.getY(), 20,
						20, null);
			}
	...
	public void PlayerBullet() {
		...
		for (int i = 0; i < this.playerAttackList.size(); i++) {
			...
			for (int j = 0; j < this.enemyUnitList.size(); j++) {
				...
				if (Crash((int) bullet.getX(), (int) bullet.getY(), enemyUnitList.get(j).x,
						enemyUnitList.get(j).y, bullet.getWidth(), bullet.getHeight(),
