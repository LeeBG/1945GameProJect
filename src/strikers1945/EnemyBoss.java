...
public class EnemyBoss extends EnemyUnit {
	...
	private void bulletCreate() {
		...
		if (bCount % 600 == 0) {
			for (int j = 1; j <= 7; j++) {
				way = 180 + (25 * j);
				bossAttack = new EnemyAttack(enemyBoss, playerPlane, enemyX + 280, enemyY + 400, way, 1, 0, 0); // 총알이 생성되는																											// 위치
				bossAttackList.add(bossAttack); // arrayList에 저장한다
			}
		}
	}
	...
	public void bossAttackProcess() {
		for (int i = 0; i < bossAttackList.size(); i++) {
			...
			// 총알이 벽과 충돌하면 사라짐
			if (bossAttack.bulletX < -10 || bossAttack.bulletX > gameFrame.SCREEN_WIDTH || bossAttack.bulletY < -10 || bossAttack.bulletY > gameFrame.SCREEN_HEIGHT) {
