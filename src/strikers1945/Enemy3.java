			...
			public void run() {
				...
				while (true) {
					try {
						...
						if (enemyY > 900) {
							System.out.println("enemy3 �벐�젅�뱶 醫낅즺");
							break;
						}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while (playerPlane.getLifeCount() > 0) {
					...
					try {
						if (collision) {
							explosePlayer(playerPlane, enemy3); // 異⑸룎 �룺諛� 硫붿꽌�뱶
						}
						Thread.sleep(10);
...
//						}
	...
	private void enemyAttack() {
		...
		if (enemyAttack.bulletX < -10 || enemyAttack.bulletX > GameFrame.SCREEN_WIDTH || enemyAttack.bulletY < -10
				|| enemyAttack.bulletY > GameFrame.SCREEN_HEIGHT) {
	...
	public void enemyDraw(Graphics g) { // 洹몃┝洹몃━湲�
		...
		for (int i = 0; i < enemyAttackkList.size(); i++) {
			enemyAttack = enemyAttackkList.get(i);
			g.drawImage(enemyAttack.bulletImg1, (int) enemyAttack.bulletX, (int) enemyAttack.bulletY,
