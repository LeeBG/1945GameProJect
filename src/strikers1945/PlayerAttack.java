...
public class PlayerAttack implements Runnable {
	...
	Image playerBulletImg2 = new ImageIcon("images/bullet3.png").getImage();
...
//			}
	...
	public boolean crash(EnemyUnit enemyUnit) { // 충돌연산
		if (Math.abs(((int)enemyUnit.getEnemyX() + enemyUnit.getEnemyWidth() / 2)
				- ((int)bulletX + enemyUnit.getEnemyWidth() / 2)) < (bulletWidth / 2 + enemyUnit.getEnemyWidth() / 2)
				&& Math.abs(((int)enemyUnit.getEnemyY() + enemyUnit.getEnemyHeight() / 2)
						- ((int)bulletY + bulletHeight / 2)) < (bulletHeight / 2 + enemyUnit.getEnemyHeight() / 2)) {
			collision = true;
