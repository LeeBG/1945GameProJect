...
public class PlayerPlane extends JLabel {
	...
	public int playerX = (GameFrame.SCREEN_WIDTH / 2) - (playerWidth / 2);
	public int playerY = (GameFrame.SCREEN_HEIGHT - (playerHeight * 2));
	...
	private void keyProcess() {
		...
		if (isDown && playerY < GameFrame.SCREEN_HEIGHT - (playerHeight + 40)) {
			playerY++;
		}
		if (isLeft && playerX > 0) {
			playerX--;
		}
		if (isRight && playerX < GameFrame.SCREEN_WIDTH - (playerWidth + 15)) {
			playerX++;
		}
	}
	...
	public void playerDraw(Graphics g) {
		for (int i = 0; i < playerAttackList.size(); i++) {
			...
			if(test == 2) {
				g.drawImage(playerAttack.playerBulletImg2, (int) playerAttack.bulletX, (int) playerAttack.bulletY, null);
			}
			if(test == 3) {
				g.drawImage(playerAttack.playerBulletImg3, (int) playerAttack.bulletX, (int) playerAttack.bulletY, null);
			}
		}
	}
...
//		
//		if ((int)playerAttack.bulletX < -10 || (int)playerAttack.bulletX > GameFrame.SCREEN_WIDTH || (int)playerAttack.bulletY < -10 || (int)playerAttack.bulletY > GameFrame.SCREEN_HEIGHT) {
