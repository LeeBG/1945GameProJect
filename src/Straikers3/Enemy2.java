...
public class Enemy2 extends EnemyUnit {
	...
	private void bulletCreate() {
		if (count % 100 == 0) {
			enemyAttack = new EnemyAttack(enemy2, playerPlane, enemyX + 20, enemyY + 40, 300, 2, 0, 0);

			// enemyAttack = new EnemyAttack(enemyX + 30, enemyY + 40);
			enemyAttackkList.add(enemyAttack);

			enemyAttack = new EnemyAttack(enemy2, playerPlane, enemyX + 80, enemyY + 40, 1300, 2, 0, 0);
	...
	public void enemyDraw(Graphics g) { // 洹몃┝洹몃━湲�
		...
		for (int i = 0; i < enemyAttackkList.size(); i++) {
			...
			g.drawImage(enemyAttack.bulletImg2, (int) enemyAttack.bulletX, (int) enemyAttack.bulletY,
					enemyAttack.b2Width, enemyAttack.b2Height, null);
